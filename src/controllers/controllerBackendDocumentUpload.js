...
const {
  documentUploadModal,
  modalViewBackendDocumentsByUniqueId,
  modalViewBackendDocumentsByReference,
} = require("../modal/modalBackendDocumentUpload");
...
exports.documentBackendUploadController = async (req, res) => {
  ...
  fs.readFile(filePath, (err, data) => {
    ...
    if (receivedDigest === computedDigest) {
      ...
      res
        .status(400)
        .json({ status: false, message: "Invalid file digest..." });
    }
  });
...
exports.controllerBackendViewByUniqueId = async (req, res) => {
  try {
    ...
    if (!token) {
      return res
        .status(400)
        .json({ status: false, message: "Header token is require." });
    }
...
exports.controllerBackendViewByReferenceNo = async (req, res) => {
  try {
    ...
    const { error, value } = validateRefNo.validate(req.body, {
      abortEarly: false,
    });
    if (error) {
      const errorMessages = error.details.map((item) => item.message); //Collection Errors
    ...
    const result = await modalViewBackendDocumentsByReference(
      referenceNo,
      token
    );
